...
async function main() {
  ...
  const agents = await Promise.all(agentDefs.map(u => prisma.user.upsert({
    where: { email: u.email }, update: {}, create: { ...u, password, role: 'agent' }
  })));
  ...
  const tags = await Promise.all(tagDefs.map(t =>
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const lead = await prisma.lead.create({
      data: {
        ...
        status, source: rand(SOURCES),
        notes: Math.random() > 0.5 ? 'Initial contact made. Interested in enterprise plan.' : null,
